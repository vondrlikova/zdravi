...
(function() {
  ...
  class GeminiClient {
    ...
    async generate(messages, systemInstruction = null, options = {}) {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          return await this._doGenerate(messages, systemInstruction, options);
        } catch (e) {
          lastError = e;
          ...
          if (typeof options.onRetry === 'function') {
            try { options.onRetry(attempt + 1, maxRetries + 1, delayMs); } catch (_) {}
          }
    ...
    _isRetryableError(e) {
      ...
      // 503 = přetížení, 429 = rate limit, síťové chyby
    ...
    async _doGenerate(messages, systemInstruction = null, options = {}) {
      ...
      // Timeout přes AbortController — 60s pro mobilní síť
  ...
  function buildSystemPrompt({ profile, recentEntries, foodsDb, herbsDb, recipes, historyItems }) {
    ...
    const historySection = historyItems?.length
...
# NEDÁVNÉ ZÁZNAMY Z DENÍKU (posledních až 20)

${recentSection}

# ULOŽENÉ RECEPTY / RITUÁLY (co si Zuzana dřív uložila jako doporučení k dodržení)

${recipesSection}

**Důležité:** Když se dotaz týká něčeho, co je v uložených receptech nebo ses o tom bavili dřív, buď **konzistentní** s předchozím doporučením. Pokud navrhuješ změnu oproti minulému, vysvětli **proč** (např. "Před 2 týdny jsem ti kokosovou vodu doporučila po cvičení při horku. Teď se ale díváš na její pravidelnou konzumaci, což je jiný kontext...").

# NEDÁVNÉ OTÁZKY (co jsme spolu už řešily — udrž konzistenci)
...
Pamatuj: jsi průvodkyně, ne lékař. Aplikace nenahrazuje péči.`;
  ...
  function formatRecipes(recipes) {
    return recipes.slice(0, 30).map(r => {
      const d = new Date(r.createdAt).toLocaleDateString('cs-CZ');
      const tagsStr = r.tags?.length ? ` [${r.tags.join(', ')}]` : '';
      // Zkrátit content na první 250 znaků, aby prompt nebyl obrovský
      const excerpt = (r.content || '').slice(0, 250).replace(/\s+/g, ' ');
      return `**${r.name}** (${d})${tagsStr}\n🎯 Na co: ${r.purpose}\nVýtah: ${excerpt}${(r.content || '').length > 250 ? '…' : ''}`;
    }).join('\n\n');
  }

  function formatHistoryItems(items) {
    // Jen titulky (a případně 100 znaků odpovědi) — plné odpovědi by prompt zbytečně nafoukly
    return items.slice(0, 20).map(h => {
      const d = new Date(h.timestamp).toLocaleDateString('cs-CZ');
      const responseExcerpt = (h.response || '').slice(0, 150).replace(/\s+/g, ' ');
      return `**${d}** ${h.icon || ''} ${h.title}\n→ ${responseExcerpt}${(h.response || '').length > 150 ? '…' : ''}`;
    }).join('\n\n');
  }
