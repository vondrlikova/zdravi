...
(function() {
  ...
  // Klíče, které se synchronizují do cloudu
  const SYNC_KEYS = new Set([
    'profile', 'entries', 'recipes', 'conversations',
    'customShortcuts', 'history'
  ]);

  const Storage = {
    _syncCallback: null,

    setSyncCallback(fn) { this._syncCallback = fn; },

    isSyncKey(key) { return SYNC_KEYS.has(key); },
    ...
    set(key, value) {
      try {
        ...
        // Zavolat sync callback pokud je nastavený a klíč je synchronizovatelný
        if (this._syncCallback && SYNC_KEYS.has(key)) {
          try { this._syncCallback(key, value); } catch (_) {}
        }
        return true;
      } catch (e) {
    ...
    /** Nastaví lokálně BEZ vyvolání sync callbacku — používá se při pull ze serveru. */
    setLocal(key, value) {
      try {
        localStorage.setItem(NS + key, JSON.stringify(value));
        return true;
      } catch (e) {
        console.error('Storage.setLocal error', key, e);
        return false;
      }
    },
    ...
    // Sync config — Supabase URL, publishable key, user_id
    getSyncConfig() { return this.get('syncConfig', {}); },
    setSyncConfig(cfg) { return this.set('syncConfig', cfg); },
